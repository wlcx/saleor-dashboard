...
filterTests({ definedTags: ["all"] }, () => {
  describe("As an admin I want to manage categories", () => {
    ...
    //     .type(category.name)
    //     .wait(["@RootCategories","@RootCategories"]);
    ...
    //   createCategory({ name: categoryName , description: categoryName }).then(categoryResp => {
    ...
    it("should be able to update category. TC: SALEOR_0207", () => {
      ...
      createCategory({ name: categoryName, description: categoryName })
        .then(categoryResp => {
          cy.log("after creation");
          cy.wait(5000);
          cy.log(categoryResp.id);
          cy.visit(urlList.categories);

          cy.visitAndWaitForProgressBarToDisappear(
            categoryDetailsUrl(categoryResp.id)
          );
          cy.log("aftern entering page");
    ...
    //   createCategory({ name: firstCategoryName, description: firstCategoryName }).then(categoryResp => {
    //     firstCategory = categoryResp;
    //   });
    //   createCategory({ name: secondCategoryName, description: secondCategoryName}).then(categoryResp => {
